    ...
    for (let i = 0; i < data.length; i++) {
      fetch("components/movieCard.html")
        ...
        .then((html) => {
          ...
          if (
            data[i].comment_title != undefined &&
            data[i].comment_title != ""
          ) {
...
document.getElementById("movieName").addEventListener("input", function () {
  ...
  if (movieName != "") {
    newMovieData["name"] = movieName;
  }
});

document
  .getElementById("movieCommentTitle")
  .addEventListener("input", function () {
    let commentTitle = document.getElementById("movieCommentTitle").value;
    if (commentTitle != "") {
      newMovieData["comment_title"] = commentTitle;
    }
  });

document
  .getElementById("commentAddMovie")
  .addEventListener("input", function () {
    let movieComment = document.getElementById("commentAddMovie").value;
    if (movieComment != "") {
      newMovieData["comment"] = movieComment;
    }
  });
...
document
  ...
  .addEventListener("click", async function () {
    ...
    crud.writeMovies(newMovieData).then((data) => {
      console.log(data);
    });
...
function confirmDelete(cardID) {
  crud.deleteMovie(cardID).then((data) => {
    console.log(data);
  });
}
...
function confirmEdit(cardID) {
  ...
  if (document.getElementById("movieCommentTitle_" + cardID).value != "") {
    newEdit["comment_title"] = document.getElementById(
      "movieCommentTitle_" + cardID
    ).value;
  }
  if (document.getElementById("commentAddMovie_" + cardID).value != "") {
    newEdit["comment"] = document.getElementById(
      "commentAddMovie_" + cardID
    ).value;
  }
  newEdit["cardID"] = cardID;
  console.log(newEdit);
  crud.updateMovie(newEdit).then((data) => {
    console.log(data);
  });
}
...
document
  .getElementById("addIMDbMovie")
  .addEventListener("click", async function () {
    let title = document.getElementById("movieNameAPI").value;
    await fetch(
      `https://www.omdbapi.com/?apikey=${keyFuncs.getKey()}&t=${title}`
    )
      ...
      .then((data) => {
        ...
        if (document.getElementById("movieCommentTitleAPI").value != "") {
          newData["comment_title"] = document.getElementById(
            "movieCommentTitleAPI"
          ).value;
        }
        if (document.getElementById("commentAddMovieAPI").value != "") {
          newData["comment"] =
            document.getElementById("commentAddMovieAPI").value;
        }
        console.log(newData);
        crud.writeMovies(newData).then((data) => {
          console.log(data);
        });
      })
      .catch((error) => {
